...
import {
  validateRequired, validateMinLength, validateMaxLength, validateEmail,
  validatePattern, validateComparison, validateExactLength
...
const AdminRegistrationForm: React.FC = () => {
  ...
  const validateField = (name: string, value: string): string => {
    switch (name) {
      case 'adminName':
        return validateRequired(value) || validateMinLength(value, 2) || validateMaxLength(value, 150);
      case 'email':
        return validateRequired(value) || validateEmail(value);
      case 'password':
        return validateRequired(value) || validateMinLength(value, 8) || validateMaxLength(value, 100);
      case 'confirmPassword':
        return validateRequired(value) || validateComparison(value, formData.password, 'Passwords do not match.');
      case 'phoneNumber':
        return validateRequired(value) || validateExactLength(value, 8) || validatePattern(value, /^\d{8}$/, 'Must contain exactly 8 digits.');
      case 'businessName':
        return validateRequired(value) || validateMinLength(value, 3) || validateMaxLength(value, 200);
      case 'subdomain':
        const subdomainRegex = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
        const cleanedValue = value.toLowerCase().replace(/[^a-z0-9-]/g, '');
        return validateRequired(cleanedValue) || validateMinLength(cleanedValue, 3) || validateMaxLength(cleanedValue, 100) || validatePattern(cleanedValue, subdomainRegex, 'Lowercase letters, numbers, and hyphens only.');
      default:
        return '';
    }
  };
  ...
  const handleBlur = (e: FocusEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const valueToValidate = name === 'subdomain'
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormData({ adminName: '', email: '', password: '', confirmPassword: '', phoneNumber: '', businessName: '', subdomain: '' });
      setClientErrors({});
      setValidationErrors({});
      ...
      if (axiosError.response?.status === 400 && axiosError.response.data?.errors) {
        ...
        setServerError(axiosError.response?.data?.detail || axiosError.response?.data?.title || axiosError.response?.data?.message || axiosError.message || "An unknown error occurred during registration.");
      }
    } finally {
      setLoading(false);
    }
   ...
   const getFieldError = (fieldName: keyof AdminRegisterDto): string | undefined => {
  ...
  return (
    <form onSubmit={handleSubmit} className={styles.form} noValidate>
      ...
      <div className={styles.formGroup}>
        ...
        <input type="text" id="adminName" name="adminName" value={formData.adminName}
               onChange={handleInputChange} onBlur={handleBlur} required
      ...
      <div className={styles.formGroup}>
        ...
        <input type="password" id="password" name="password" value={formData.password}
               onChange={handleInputChange} onBlur={handleBlur} required minLength={8} maxLength={100}
      ...
      <div className={styles.formGroup}>
       ...
       <div className={styles.formGroup}>
        ...
        <input type="tel" id="phoneNumber" name="phoneNumber" value={formData.phoneNumber}
               onChange={handleInputChange} onBlur={handleBlur} required maxLength={8} pattern="\d{8}"
               title="Must be exactly 8 digits" aria-invalid={!!getFieldError('phoneNumber')} />
      ...
      <div className={styles.formGroup}>
        ...
        <input type="text" id="businessName" name="businessName" value={formData.businessName}
               onChange={handleInputChange} onBlur={handleBlur} required minLength={3} maxLength={200}
      ...
      <div className={styles.formGroup}>
        ...
        <input type="text" id="subdomain" name="subdomain" placeholder="e.g., mybakery" value={formData.subdomain}
               onChange={handleInputChange} onBlur={handleBlur} required minLength={3} maxLength={100}
               pattern="^[a-z0-9]+(?:-[a-z0-9]+)*$" title="Lowercase letters, numbers, and hyphens only."
      ...
      {serverError && !successMessage && <p className={styles.error}>{serverError}</p>}
