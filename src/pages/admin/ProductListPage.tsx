...
const ProductListPage: React.FC = () => {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            if (axiosError.response?.status === 401) {
                ...
                setError(axiosError.response?.data?.title || axiosError.message || "Failed to load products.");
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleToggleAvailability = async (productId: string, currentAvailability: boolean) => {
         ...
         try {
             console.log(`Setting availability for ${productId} to ${!currentAvailability}`);
             await axios.patch(`${apiUrl}/products/${productId}/availability`, !currentAvailability, {
             ...
             alert("Failed to update product availability. " + (axiosError.response?.data?.title || axiosError.message));
         }
    };
    ...
    return (
        <div>
            ...
            <p>For tenant: {user?.administeredTenantSubdomain ?? user?.administeredTenantId ?? 'N/A'}</p>
                         ...
                         <tbody>
                             {products.map(prod => (
                                 <tr key={prod.id}>
                                     ...
                                     <td>
                                         <button onClick={() => navigate(`/admin/products/edit/${prod.id}`)} style={{marginRight: '5px'}}>Edit</button>
                                         <button onClick={() => handleToggleAvailability(prod.id, prod.isAvailable)}>
