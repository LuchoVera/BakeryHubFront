...
const AddCategoryForm: React.FC<AddCategoryFormProps> = ({ onCategoryAdded }) => {
        ...
        try {
            ...
            if (axiosError.response?.status === 401) {
                ...
                setTimeout(() => {
                 ...
                 setError(axiosError.response?.data?.title || axiosError.message || "Failed to load categories.");
            }
             setLoading(false); 
        } finally {
             setLoading(false);
    ...
    return (
        <form onSubmit={handleSubmit} style={{ margin: '20px 0', display: 'flex', gap: '10px', alignItems: 'center' }}>
            <input
                type="text"
